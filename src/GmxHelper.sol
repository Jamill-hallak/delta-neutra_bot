...
abstract contract GmxHelper {
    ...
    function getSizeDeltaUsd(
        ...
        if (isIncrease) {
            uint256  newAmount = collateralAmount + longTokenAmount ;
             uint256 newSizeUSD = newAmount * longTokenPrice 
               * 10 ** (30 - longTokenDecimals - CHAINLINK_DECIMALS); 
               if(newSizeUSD > sizeInUsd){
               sizeDeltaUsd = newSizeUSD -sizeInUsd ;}
            ...
            uint256 newAmount = collateralAmount - longTokenAmount ;
             uint256 newSizeUSD = newAmount * longTokenPrice 
               * 10 ** (30 - longTokenDecimals - CHAINLINK_DECIMALS); 
            if(newSizeUSD < sizeInUsd){
               sizeDeltaUsd = sizeInUsd -newSizeUSD  ;}
            // new position size = long token price * new collateral amount
            // new collateral amount = position.collateralAmount - longTokenAmount
            // sizeDeltaUsd = new position size - position.sizeInUsd
        }
    ...
    function createIncreaseShortPositionOrder(
       ...
       uint256 positionSizeDelta = getSizeDeltaUsd( longTokenPrice,
        ...
        exchangeRouter.sendTokens(address(longToken), ORDER_VAULT, longTokenAmount);

        uint256 acceptablePrice = longTokenPrice *1e12/CHAINLINK_MULTIPLIER* 90/100 ;

        return exchangeRouter.createOrder(IBaseOrderUtils.CreateOrderParams({
    ...
    numbers : IBaseOrderUtils.CreateOrderParamsNumbers({
        sizeDeltaUsd : positionSizeDelta ,
    ...
    function createDecreaseShortPositionOrder(
        ...
        uint256 postionSizeDelta = getSizeDeltaUsd( longTokenPrice,
            position.numbers.sizeInUsd,
            position.numbers.collateralAmount,
            longTokenAmount,
            false );
        ...
        uint256 acceptablePrice = longTokenPrice *1e12/CHAINLINK_MULTIPLIER *110/100 ;
    return exchangeRouter.createOrder(IBaseOrderUtils.CreateOrderParams({
        addresses :IBaseOrderUtils.CreateOrderParamsAddresses({
        receiver : receiver ,
        cancellationReceiver : address(0) ,
