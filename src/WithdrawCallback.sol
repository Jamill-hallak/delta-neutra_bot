...
contract WithdrawCallback is Auth {
    ...
    function refundExecutionFee(
        ...
        address refundAddress = refunds[key] ;

        require(refundAddress !=address(0),"worng address");
        delete refunds[key];
    
          uint256 bal = address(this).balance;
        if (bal > 0) {  
        (bool success, ) = refundAddress.call{value: bal}("");
    ...
    function afterOrderExecution(
       ...
       IVault.WithdrawOrder memory  withdrawOrder = vault.getWithdrawOrder(key) ;
        require(withdrawOrder.account !=address(0),"worng address");
        require(order.numbers.orderType == Order.OrderType.MarketDecrease,"worng ordertype");
    ...
    function afterOrderCancellation(
        ...
        IVault.WithdrawOrder memory  withdrawOrder = vault.getWithdrawOrder(key) ;
        require(withdrawOrder.account !=address(0),"invalid key ");
        require(order.numbers.orderType == Order.OrderType.MarketDecrease,"worng ordertype");
        setRefundAccount(key,withdrawOrder.account);
