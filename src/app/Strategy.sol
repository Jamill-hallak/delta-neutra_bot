...
contract Strategy is Auth, GmxHelper {
    ...
    function totalValueInToken() external view returns (uint256) {
       ...
       int256 positionval = getPositionWithPnlInToken() ;
        if(positionval>0){
            val+=uint256(positionval) ;
        }
        else {
         val -= Math.min(val, uint256(-positionval));
         }
       return  val ;
    }
    ...
    {
        return createIncreaseShortPositionOrder( msg.value  ,wethAmount ) ;
    }
   ...
   orderKey= createDecreaseShortPositionOrder({
        ...
        callbackGasLimit :0
            ...
            uint256 longtokeamount = getPositionCollateralAmount() *wethAmount /uint256(total) ;

    orderKey= createDecreaseShortPositionOrder({
         executionFee :msg.value,
         longTokenAmount :longtokeamount,
